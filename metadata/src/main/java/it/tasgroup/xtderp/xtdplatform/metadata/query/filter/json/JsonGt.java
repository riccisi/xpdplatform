 ...
 * @since 1.0
 */
final class JsonGt extends JsonExpression {

    JsonGt(JsonNode node) {
        super(node);
    }

    @Override
    public void applyOn(Statement stmt) {
        stmt.gt(this.property(), Comparable.class.cast(this.value()));
    }
    ...
    static final class Matcher implements FilterMatcher {
        ...
        public boolean match() {
            ...
            return !operator.isMissingNode() && Arrays.asList("gt", ">").contains(operator.asText());
        }

        @Override
        public Filter matched() {
            return new JsonGt(this.node);
        }
    }
}
